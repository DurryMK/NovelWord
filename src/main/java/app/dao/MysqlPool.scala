...
class MysqlPool extends Serializable {   //可序列化
  ...
  try {
    ...
    cpds.setPassword(conf.get("password").getOrElse("sys123"))
    ...
    // 检测连接配置
    cpds.setPreferredTestQuery("select id from logs limit 1")
...
object MysqlManager {
  ...
  def main(args: Array[String]): Unit = {
    var con = getMysqlManager
    println(con.getConnection)
  }
}
